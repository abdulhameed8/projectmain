    ...
    public class TenantsController : ControllerBase
    {
            ...
            [FromQuery] int pageSize = 10)
        {
            try
            {
                ...
                var (tenants, totalCount) = await _unitOfWork.Tenants.GetTenantsPagedAsync(
            ...
            string tenantCode)
        {
            try
            {
                _logger.LogInformation("Fetching tennat with code {TennatCode} for tenant {TenantId}",
                    tenantCode, tenantId);
                ...
                if (tenant == null)
                {
                    _logger.LogWarning("Tennat with code {TenantCode} not found for tenant {TenantId}",
                        tenantCode, tenantId);
                ...
                _logger.LogInformation("Successfully retrieved tenant with code {TennatCode}", tenantCode);
        ...
        public async Task<ActionResult<ApiResponse<TenantDto>>> CreateTenant([FromBody] CreateTenantdto createTenantdto)
        {
            try
            {
                _logger.LogInformation("Creating new tenant with code {TennatCode} for tenant {TenantId}",
                ...
                if (!validationResult.IsValid)
                {
                    ...
                    _logger.LogWarning("Validation failed for tennat creation: {Errors}",
                        string.Join(", ", errors));
                ...
                if (!isUnique)
                {
                    _logger.LogWarning("Tenant code {TennatCode} already exists for tenant {TenantId}",
                        createTenantdto.TenantCode, createTenantdto.TenantId);
        ...
        [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<ApiResponse<List<TenantDto>>>> SearchTenants(
            [FromQuery] Guid tenantId,
            [FromQuery] string searchTerm)
        {
            try
            {
                ...
                var tenants = await _unitOfWork.Tenants.SearchTenantsAsync(tenantId, searchTerm);
                var tenantDtos = _mapper.Map<List<TenantDto>>(tenants);

                _logger.LogInformation("Found {Count} tenant matching search term", tenantDtos.Count);
        ...
        [ProducesResponseType(typeof(ApiResponse<List<TenantDto>>), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<ApiResponse<List<TenantDto>>>> GetActiveTenants([FromQuery] Guid tenantId)
        {
            try
            {
                ...
                var tenants = await _unitOfWork.Tenants.GetActiveTenantsAsync(tenantId);
                var tenantDtos = _mapper.Map<List<TenantDto>>(tenants);

                _logger.LogInformation("Successfully retrieved {Count} active tenants", tenantDtos.Count);

                return Ok(new ApiResponse<List<TenantDto>>(tenantDtos, "Active tenants retrieved successfully"));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error occurred while fetching active tenants for tenant {TenantId}", tenantId);
                throw;
            }
        }
    }
